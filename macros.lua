...
function update_overlay_state()
    ...
    elseif overlay_state == VOLUME_OVERLAY then
        ...
        -- fill background
        for i = 1, num_keys do
            color_map_overlay[i] = rgb_to_color(235, 0, 250)
        end

        -- render volume level as highlight
        for i = 1, num_keys do
            if i < upper_bound then
                color_map_overlay[i] = rgb_to_color(255, 0, 0)
            end
        end
    end
end
    ...
    if ticks % animation_delay == 0 or force_update then
        ...
        for i = 1, num_keys do
            ...
            if overlay_ttl > 0 then
                ...
                overlay_ttl = 0
                color_map_overlay[i] = 0xff000000
            end
        end
        ...
        overlay_ttl = overlay_ttl - max(1.0, ((overlay_ttl / overlay_step) / 1))
